...
const genData = () => {
  const data: RulePackageInstance = mockjs.mock({
    id: '@id',
    name: '@csentence',
...
const genList = (current: number, pageSize: number) => {
  const tableListDataSource: Message[] = [];
...
function queryList(req: Request, res: Response, u: string): ApiResponsePage<Message> {
  let realUrl = u;
  ...
  const result: ApiResponsePage<RulePackageInstance> = {
    code: 200,
    status: true,
    data: {
      list: dataSource,
...
export default {
  'POST /rule/queryPackageInstances': queryList,
};
